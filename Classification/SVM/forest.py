...
df = pd.read_csv("input/new_results.csv")
df = df[df["FP"] + df["TP"] >= 2]
collist = df["Feature"].tolist()

df = pd.read_csv("input/gene_results.csv")
...
df = pd.read_csv("input/gene_results.csv", index_col=0)
df = df.loc[intlist]
...
utedata = pd.read_csv('input/gene_level.csv', index_col=0)
...
while tf.empty:
    ...
    for parm in runtype:
        ...
        totalmean.append([parm[0] + " : Other"] + tfmean)
